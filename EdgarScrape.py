...
class GetFilings:
    ...
    def get_xml_file(self, html, i):
        s = BS(html, "lxml")
        try:
            xml_link = s.find_all('table', {'class': 'tableFile', 'summary': 'Data Files'})[
                0].find_all('tr')[i].find_all('td')[2].find('a')['href']
            xtname = ['.xml', '.xsd']
    ...
    def get_txt(self, html):
        s = BS(html, "lxml")
        try:
            txt_link = s.find_all('table', {
                                  'class': 'tableFile', 'summary': 'Document Format Files'})[0].find_all('td')
            link_idx = None
    ...
    def get_xl(self, html):
        s = BS(html)
        try:
            xl_link = s.find_all('td', {'colspan': '2'})[
                0].find_all('a')[1]['href']
    ...
    def get_all_10q(self):
        ...
        errors = {}
        success = {}
        for link_val in tqdm(self.filings['10q_list'], desc="10-Q: HTML"):
        ...
        if settings.GET_XL:
            ...
            for link_val in self.filings['10q_xl_list']:
                ...
                if link_xl:
                    self.filings['10q_xl'].append(
                        (link_xl, dates[count], 'xl'))
                    try:
    ...
    def download_all(self):
        ...
        # Download HTML files
        if settings.GET_HTML:
            for link in tqdm(self.filings['10q_html'], desc="10-Q"):
                fname = "{0}_{1}_{2}.html".format(
                    self.ticker_symbol, link[1], link[2])
                diry = '{0}/{1}/html/10-Q/{2}/'.format(
                    settings.RAW_DATA_PATH, self.ticker_symbol, link[1])
                if not os.path.exists(diry):
                    os.makedirs(diry)
                if not self.check_duplicate(diry, fname):
        ...
        if settings.GET_TXT:
            for link in self.filings['10q_txt']:
                fname = "{0}_{1}_{2}.txt".format(
                    self.ticker_symbol, link[1], link[2])
                diry = '{0}/{1}/txt/10-Q/{2}/'.format(
                    settings.RAW_DATA_PATH, self.ticker_symbol, link[1])
                if not os.path.exists(diry):
                    os.makedirs(diry)
                if not self.check_duplicate(diry, fname):
        ...
        if settings.GET_XL:
            for link in self.filings['10q_xl']:
                fname = "{0}_{1}_{2}.xlsx".format(
                    self.ticker_symbol, link[1], link[2])
                fname = self.ticker_symbol + fname
                diry = '{0}/{1}/xl/10-Q/{2}/'.format(
                    settings.RAW_DATA_PATH, self.ticker_symbol, link[1])
                if not os.path.exists(diry):
                    os.makedirs(diry)
                if not self.check_duplicate(diry, fname):
